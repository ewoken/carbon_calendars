...
async function fetchYearData(year) {
  const res = await fetch(
    `https://energy-charts.info/charts/power/raw_data/de/year_${year}.json`
  );
  const d = await res.json();

  const f = d.map((typeData, i) => {
    const type = year == 2010 && i >= 8 ? TYPES[i + 1] : TYPES[i];
    const values = typeData.values.map((t) => ({
      [type]: Math.round(t[1] * 1000),
    }));

    return values;
  });
  const g = f[0]
    .map((_, i) => f.map((row) => row[i]))
    .map((items) => merge(...items));

  const startYear = moment().year(year).startOf("year");
  const unit = year == 2019 ? "minutes" : "hours";
  const step = year == 2019 ? 15 : 1;
  return g.map((item, i) => ({
    startDate: moment(startYear)
      .add(i * step, unit)
      .format(FORMAT),
    endDate: moment(startYear)
      .add((i + 1) * step, unit)
      .format(FORMAT),
    others: 0,
    ...item,
...
async function main() {
  ...
  years.forEach((year, i) => {
    fs.writeFileSync(`./data/germany/${year}.csv`, toCSV(yearData[i]));
  });
}
